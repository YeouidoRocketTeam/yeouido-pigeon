...
import {
  Info, Shield, Database, GitBranch, Clock, Eye, CheckCircle2,
...
const CRITERIA = [
  ...
  },
  {
    key: "time_validity",
    label: "시점 유효성",
    subtitle: "지금 바로 써먹을 수 있는 최신 정보인가요?",
    weight: 0.242,
    icon: Clock,
    checklist: [
  ...
  },
  {
    ...
    label: "이해관계 투명성",
    subtitle: "광고나 홍보 목적을 숨기고 있지는 않나요?",
    weight: 0.103,
    icon: Eye,
    checklist: [
  ...
  },
  {
    ...
    label: "데이터 구체성",
    subtitle: "추측이 아닌 실제 숫자나 도표가 있나요?",
    weight: 0.061,
    icon: Database,
    checklist: [
  ...
  },
  {
    ...
    label: "교차 검증 일치도",
    subtitle: "다른 믿을만한 곳들도 같은 말을 하나요?",
    weight: 0.027,
    icon: CheckCircle2,
    checklist: [
                ...
                <div key={c.key} className="rounded-lg border bg-background">
                  ...
                  {isExpanded && (
                    <div className="px-3 pb-3 pt-0">
                      <p className="text-xs text-muted-foreground mb-2 pl-7">{c.subtitle}</p>
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-5 mt-2">
            <div>
              <h3 className="text-sm font-semibold text-foreground mb-2">📐 산출 공식 (ROC 가중치법)</h3>
              <div className="rounded-lg border-l-4 border-primary bg-primary/5 px-4 py-3">
                <p className="text-sm font-medium text-foreground">
                  최종 점수 = Σ (항목별 점수 / 100 × 가중치) × 100
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  각 항목은 5개 체크리스트(30/25/20/15/5점 감점)로 평가, 최저 5점 보장
                </p>
              </div>
            </div>

            <div className="space-y-3">
              <h3 className="text-sm font-semibold text-foreground">📊 6대 평가 항목</h3>
              {CRITERIA.map((c) => {
                const Icon = c.icon;
                return (
